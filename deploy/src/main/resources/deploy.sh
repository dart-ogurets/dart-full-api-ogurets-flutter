...
echo helm upgrade oguretsexample oguretsexample -f oguretsexample/${ENV}-values.yaml --debug -i
# this is always causing it to go into pending-install instead of deployed
helm upgrade oguretsexample oguretsexample -f oguretsexample/${ENV}-values.yaml -f manifest.yaml --debug -i
